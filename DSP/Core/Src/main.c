...
char buffer[1024]; // In order to store data
UINT br,bw; // Will be needed when doing reading and writing




/* ---------------------------Private define -------------------*/
#define SAMPLING_TIME 1024 // Block Size
#define NUM_TAPS 24
#define True 1
...
uint32_t blockSize = SAMPLING_TIME;


/* -------- Defining arm_f32_fir variables-------- */


float32_t firState32[SAMPLING_TIME + NUM_TAPS -1]; /**/

const float32_t firCoeffs32[NUM_TAPS] = { /* This is my filter */
  ...
  -0.008369651092956, -0.03089272458715, -0.05025621157586, -0.04394831977039,
   ...
   -0.04394831977039, -0.05025621157586, -0.03089272458715,-0.008369651092956,
    0.00434530488403, 0.006527502826968, 0.003775348456944
};
...
int bufsize(char *buf) // Calculating the length of the data
{
	int i = 0;
	while (*buf++ != '\0') //Stop when you find the null
		i++;
	return i;
}
